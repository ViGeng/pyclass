...
def print_class_sheet(class_sheet):
    ...
    print('%3s'%'ID','','%-8s'%'Name','%-3s'%'Score')
    for i in range (len(class_sheet_id)):
        t=class_sheet_id[i]
        print('%3s'%class_sheet_id[i],'','%-8s'%class_sheet[t][0],'%-3s'%class_sheet[t][1])
    pass
...
def print_class_sheet_sorted(class_sheet):
    ...
    """
    class_sheet_sorted=sorted(class_sheet.items(),key=lambda item:item[1][1])
    print('%3s'%'ID','','%-8s'%'Name','%-3s'%'Score')
    for i in range (len(class_sheet_sorted)):
        m=class_sheet_sorted
        print('%3s'%m[i][0],'','%-8s'%m[i][1][0],'%-3s'%m[i][1][1])
    pass
