...
def print_class_sheet(class_sheet):
    ...
    """
    sorted_class_sheet = sorted(class_sheet.items(), key=lambda x: x[0])
    print(' ID    Name          Score')
    for item in sorted_class_sheet:
        print('%3d    %-14s%-3d' %(item[0], item[1][0], item[1][1]))

def print_class_sheet_sorted(class_sheet):
    """ 打印成绩单，按照成绩排序，每行包括ID、姓名和期末考试成绩，要求每行能够对齐
    """
    sorted_class_sheet = sorted(class_sheet.items(), key=lambda x: x[1][1])
    print(' ID    Name          Score')
    for item in sorted_class_sheet:
        print('%3d    %-14s%-3d' % (item[0], item[1][0], item[1][1]))
