...
def print_class_sheet(class_sheet):
    ...
    id_sheet = sorted(class_sheet.items(),key=itemgetter(0))
    print('\tID\tName\t\tScore')
    for key,value in id_sheet:
...
def print_class_sheet_sorted(class_sheet):
    ...
    """
    from operator import itemgetter 
    score_sheet = sorted(class_sheet.items(),key=lambda x: x[1][1])
    print('\tID\tName\t\tScore')
    for key,value in score_sheet:
        print('\t%d\t%s\t\t%d' % (key,value[0],value[1]))
