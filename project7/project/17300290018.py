...
def get_class_sheet(total=25):
    ...
    for i in range(total):
        ID = int(util.get_id())
        class_sheet[ID] = [util.get_name(), util.get_score()]
...
def print_class_sheet(class_sheet):
    ...
    """
    class_sheet_sort = sorted(class_sheet.items(), key = lambda x:x[0])
    print(" ID  Name        Score")
    for key, value in class_sheet_sort:
...
def print_class_sheet_sorted(class_sheet):
    ...
    """
    class_sheet_sort = sorted(class_sheet.items(), key = lambda x:x[1][1])
    print(" ID  Name        Score")
    for key, value in class_sheet_sort:
        print ('%3d  %-10s  %d' % (key, *value))
