...
def get_class_sheet(total=25):
    ...
    for _ in range(total):
        ...
        ID = util.get_id()
        name = util.get_name()
        score = util.get_score()#调用函数，形成25个ID,name,score
        
        class_sheet[ID] = (name, score)#保存在字典中
...
def print_class_sheet(class_sheet):
    ...
    list1 = sorted(class_sheet.items(), key = lambda d:d[0])
        
    for item in list1:
        print(f"{item[0]:-4}\t{item[1][0]:12}{item[1][1]}")        
    pass
...
def print_class_sheet_sorted(class_sheet):
    ...
    """
    list2 = sorted(class_sheet.items(), key = lambda d: d[1][1])
    for item in list2:
        print(f"{item[0]:-4}\t{item[1][0]:12}{item[1][1]}")
    pass
