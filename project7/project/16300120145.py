...
def get_class_sheet(total=25):
    ...
    for _ in range(total):
        s_id = util.get_id()
        s_name = util.get_name()
        s_score = util.get_score()
        class_sheet[int(s_id)] = class_sheet.get(int(s_id),[])
        class_sheet[int(s_id)] += [str(s_name),int(s_score)]
        pass
...
def print_class_sheet(class_sheet):
    ...
    """
    class_sheet = sorted(class_sheet.items(), key=lambda item: item[0])
    print('%-5s %-10s %-5s' % ('ID','Name','Score'))
    for i in class_sheet:
      print('%-5s %-10s %-5s' %(i[0],i[1][0],i[1][1])) 
    pass
