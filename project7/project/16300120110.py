...
def get_class_sheet(total=25):
    ...
    for _ in range(total):
        # your code here....
        nameList=util.get_name()
        scoreList=util.get_score()
        idList=util.get_id()
        class_sheet[idList] = [nameList,scoreList]
...
def print_class_sheet(class_sheet):
    ...
    for key in sorted(class_sheet.keys()):
        print("%3d     %-10s        %3d"%(key,class_sheet[key][0],class_sheet[key][1]))
    return
...
def print_class_sheet_sorted(class_sheet):
    ...
    sorted_class_sheet = sorted(class_sheet.items(), key=lambda kv: kv[1][1])
    #print(sorted_class_sheet)
    for i in range(len(sorted_class_sheet)):
        print("%3d     %-10s        %3d"%(sorted_class_sheet[i][0],sorted_class_sheet[i][1][0],sorted_class_sheet[i][1][1]))
    return
