...
def print_class_sheet(class_sheet):
    ...
    print('%3s  %-10s  %-5s'%('ID','Name','Score'))
    listsort = sorted(class_sheet)
    for i in range(len(class_sheet)):
        now_key = listsort[i]
        print('%3s  %-10s  %-5s'%(now_key,class_sheet[now_key][0],class_sheet[now_key][1]))
...
def print_class_sheet_sorted(class_sheet):
    ...
    print('%3s  %-10s  %-5s'%('ID','Name','Score'))
    listsort = sorted(class_sheet.items(),key = lambda x:x[1][1])
    for i in range(len(class_sheet)):
        now_l = listsort[i]
        print('%3d  %-10s  %-5s'%(now_l[0],now_l[1][0],now_l[1][1]))
