...
def get_class_sheet(total=25):
    ...
    for i in range(total):
        ...
        class_sheet.setdefault(ids,[]).append(name)
        class_sheet.setdefault(ids,[]).append(score)
        i += 1
        pass
...
def print_class_sheet(class_sheet):
    ...
    a = sorted(class_sheet.items(),key = operator.itemgetter(0))
    print(('%5s   %-15s %-10s')%('ID', 'Name', 'Score'))
    n = 0
    for n in range(25):
        print(('%5d   %-15s %-10d')%(a[n][0], a[n][1][0], a[n][1][1]))
        n += 1
...
def print_class_sheet_sorted(class_sheet):
    ...
    b = sorted(class_sheet.items(),key = lambda item:(item[1][1]))
    print(('%5s   %-15s %-10s')%('ID', 'Name', 'Score'))
    n = 0
    for n in range(25):
        print(('%5d   %-15s %-10d')%(b[n][0], b[n][1][0], b[n][1][1]))
        n += 1
    pass
