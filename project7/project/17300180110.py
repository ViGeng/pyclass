...
def get_class_sheet(total=25):
    ...
    for i in range(total):
        class_sheet.setdefault(util.get_id(),[]).extend((util.get_name(),util.get_score()))
...
def print_class_sheet(class_sheet):
    ...
    """
    sheet=list(class_sheet.items())
    sheet.sort(key=lambda item:item[0])
    print('ID\tName\t        Score')
    for i in range(len(sheet)):
        print('%3d\t%-10s\t%3d' % (sheet[i][0],sheet[i][1][0],sheet[i][1][1]))
...
def print_class_sheet_sorted(class_sheet):
    ...
    """
    sheet=list(class_sheet.items())
    sheet.sort(key=lambda item:item[1][1])
    print('ID\tName\t        Score')
    for i in range(len(sheet)):
        print('%3d\t%-10s\t%3d' % (sheet[i][0],sheet[i][1][0],sheet[i][1][1]))
