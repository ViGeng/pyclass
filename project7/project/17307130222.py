...
def print_class_sheet(class_sheet):
    ...
    print('%3s   %-10s  %-10s'  %('ID','Name','Score'))
    
    for key,value in class_sheet.items():
        print('%3s   %-10s  %-10s' %(key,value[0],value[1]))
    pass
...
def print_class_sheet_sorted(class_sheet):
    ...
    sorted_class_sheet= dict(sorted(class_sheet.items(), key=lambda d:d[1][1]))

    for key,value in sorted_class_sheet.items():
        print('%3s   %-10s  %-10s' %(key,value[0],value[1]))
    pass
