...
def get_class_sheet(total=25):
    ...
    values = []
    for i in range(total):
        values.append([util.get_name(),util.get_score()])
    class_sheet = dict(zip(keys,values))
    return class_sheet

get_class_sheet(total=25)
...
def print_class_sheet(class_sheet):
    ...
    print('{0:5}{1:15}{2:3}'.format('id', 'name','score'))
    order1 = sorted(class_sheet.items())
    for item in order1:
        a = item[0]
        b = item[1][0]
        c = item[1][1]
        print('{0:<5}{1:15}{2:3}'.format(a,b,c))
...
def print_class_sheet_sorted(class_sheet):
    ...
    def keyorder(item):
        return item[1][1]
    order2 = sorted(class_sheet.items(),key = keyorder)
    for item in order2:
        a = item[0]
        b = item[1][0]
        c = item[1][1]
