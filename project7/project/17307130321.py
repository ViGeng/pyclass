...
def get_class_sheet(total=25):
    ...
    for i in range(total):
        class_sheet.setdefault(util.get_id(),[]).extend([util.get_name(min_chars=3,max_chars=8),util.get_score()])
    return class_sheet
...
def print_class_sheet(class_sheet):
    ...
    """
    d1=[('ID',['Name','Score'])]
    d2=sorted(class_sheet.items(),key=lambda d:d[0],reverse=False)
    dict1=d1+d2;
    for item in dict1:
        print('%5s   %-16s%-s' %(item[0],item[1][0],item[1][1]))
           
def print_class_sheet_sorted(class_sheet):
    """ 打印成绩单，按照成绩排序，每行包括ID、姓名和期末考试成绩，要求每行能够对齐
    """
    d3=[('ID',['Name','Score'])]
    d4=sorted(class_sheet.items(),key=lambda d:d[1][1],reverse=False)
    dict2=d3+d4;
    for item in dict2:
        print('%5s   %-16s%-s' %(item[0],item[1][0],item[1][1]))
