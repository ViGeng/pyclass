...
def decorate_text(text):
    line = text.splitlines(keepends=False)
    line_width = []
    new_line = []
    
    for i in line:
        new_i = i.rstrip()
        new_line.append(new_i)
        i_width = len(new_i)
        line_width.append(i_width)
    
    width_standard = max(line_width)
    print('+'+'-'*width_standard+'+')
    
    for i in new_line:
        char_fill = width_standard - len(i) +1
        #'|'占一个长度
        print('{0:s}{1:s}{0:>{2}}'.format('|',i, char_fill))
        #print('|'+i.ljust(width_standard,' ')+'|')
        #ljust方法让文本部分内部调整，所以不用考虑'|'的占位
